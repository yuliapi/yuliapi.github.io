var HTMLskills = '<td class="col-md-8 col-xs-8">%data%</td>';
var HTMLparagraph = '<p class="text-justify text-indent mx-4">%data%</p>';
var HTMLgalleryStart = '<div class = "col-md-4 project-entry"></div>';
var HTMLgalleryImage =  '<img class="img-responsive center-block">';
var HTMLgalleryTitle = '<h3 class="text-uppercase text-medium">%data%</h3>';
var HTMLgalleryLink = '<a class="inline-block"  target="_blank">%data%</a>';
var HTMLgalleryText = '<div style = "padding: 0 10px; width: 370px; margin: 0 auto"><p class="text-justify text-indent mx-4">%data%</p></div>';


var HTMLexperienceTerm =  '<td class="term-ed "><h3>%data%</h3></td>';
var HTMLexperienceCompany = '<h3>%data%</h3>';
var HTMLexperiencePosition = '<p><em>%data%</em></p>';


var  HTMLeducationTerm = '<div class="term-ed"><h5>%data%</h5></div>';
var HTMLeducationCompany = '<h5>%data%</h5>';
var HTMLeducationPosition = '<p><small>%data%</small></p>';


var HTMLlistItem = '<li>%data%</li>';
var HTMLparagraph = '<p>%data%</p>';
var HTMLRow = '<div class="row"></div>';
